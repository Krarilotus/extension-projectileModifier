...
function M.new(blocks, config)
    ...
    function state:deserialize(handle)
        ...
        assert(handle:get('format') == '1', '[projectileModifier] unsupported saved state format')
        assert(handle:get('config') == self.config, '[projectileModifier] saved projectile settings differ; restore the settings used for this save')
        local pending = {}
        for i, block in ipairs(self.blocks) do
            local name = block[1] .. '.bin'
            assert(handle:exists(name), '[projectileModifier] missing saved state: ' .. name)
            local bytes = handle:get(name)
            assert(type(bytes) == 'string' and #bytes == block[3], '[projectileModifier] invalid saved state: ' .. name)
            local limits = {cooldown=60000, movement=20, pending=63, ['pending-cooldown']=60000, ['sync-wait']=60000}
            if limits[block[1]] then
                for offset = 1, #bytes, 4 do
                    local a,b,c,d = bytes:byte(offset, offset+3)
                    local value = a + b*256 + c*65536 + d*16777216
                    assert(value <= limits[block[1]], '[projectileModifier] out-of-range saved state: ' .. name)
                end
            end
            pending[i] = bytes
        end
